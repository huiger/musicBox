...
IconDart = 'lib/common/style/icon_fonts.dart'
# iconfont css 文件存放路径
...
{icon_codes}
    ...
    for match in matchObj:
        name = match.group(1)
        name = name.replace("-","_")
        string = f'  static const IconData {name} = IconData(0x{match.group(3)}, fontFamily: IconFonts.FONT_FAMILY);'
        strings.append(string)
    strings = '\n'.join(strings)
    code = code.replace('{icon_codes}', strings)
    code = code.replace('{date}', time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
    open(MAIN / IconDart, 'w', encoding='utf-8').write(code)
